	...
	<div class="AreaSelectModal" ref="AreaSelector" :style="styleOption" v-if="option.isShow">
        ...
        <div class="data-list">
            <ul class="clearfix" ref="province" v-show="showProvinceList">
                <li v-for="(province,i,index) in provinceList" @click.stop="selectProvince(province,i)">
                	<span v-text="province"></span>
                </li>
            </ul>
            <ul class="clearfix" ref="city" v-show="showCityList">
            	<li class="all-area" @click.stop="hideAreaSelector()">
            		<span v-text="selectedArea.province.value"></span>
            	</li>
                <li v-for="(city,i,index) in cityList" @click.stop="selectCity(city,i)">
                	<span v-text="city"></span>
                </li>
            </ul>
            <ul class="clearfix" ref="dist" v-show="showDistList">
            	<li class="all-area" @click.stop="hideAreaSelector()">
            		<span v-text="selectedArea.city.value"></span>
            	</li>
                <li v-for="(dist,i,index) in distList" @click.stop="selectDist(dist,i)">
                	<span v-text="dist"></span>
                </li>
            </ul>
        </div>
	</div>
