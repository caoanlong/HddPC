<template>
	<div>
							...
							<div class="bd">
								...
								<div class="tabCon">
								    ...
								    <div class="form-item">
								    	...
								    	<button :disabled="$v.mobile.$invalid||isGetVCode" v-text="getVcodeText" @click="getVcode()" class="GetVcode"></button>
								    </div>
								    ...
								    <div class="form-item checkboxItem" v-show="!isLogin">
							        	...
							        	<span class="c2 agreementBtn" @click="isShowAgreement = true">《货多多注册条款》</span>
							        </div>
								    <div class="form-item">
										<button :disabled="$v.validationGroup.$invalid" type="button" v-show="isLogin" @click="signin()" class="btn loginBtn">登录</button>
		...
		<transition name="fade">
			<CustomModal title="《货多多注册条款》" button1="确定" @closeModal="closeAgreementModal" v-if="isShowAgreement">
	    ...
	    <transition name="fade">
		    <CustomModal :icon="2" @closeModal="closeRegisterModal" @doSomething="goCompleteInfo" button1="完善资料" button2="继续逛逛" v-if="isShowRegister">
		    	<p class="f14" style="padding:5px 0">注册成功</p>
				<p style="padding:5px 0">为了方便您的后续操作</p>
				<p style="padding:5px 0">请完善资料</p>
		    </CustomModal>
	    </transition>
	    <transition name="fade">
		    <CustomModal :icon="2" @closeModal="closeSigninModal" @doSomething="goCompleteInfo" button1="完善资料" button2="继续逛逛" v-if="isShowSignin">
		    	<p class="f14" style="padding:5px 0">登录成功</p>
				<p style="padding:5px 0">为了方便您的后续操作</p>
				<p style="padding:5px 0">请完善资料</p>
		    </CustomModal>
	    </transition>
		...
		methods: {
			...
			getVcode() {
				...
				this.$http.post(URL, params).then(
					(res) => {
						this.showMsg({msg: '短信验证码：'+res.body.data, delay: 3000});
			...
			register() {
				...
				this.$http.post(URL,params).then(
					(res) => {
						if (res.body.code == 200) {
							...
							if (res.body.data.certifyStatus == 'Y') {
								this.showMsg({msg: '注册成功',delay: 1000});
					...
					(res) => {
						this.showMsg({msg: '请求失败',delay: 1000});
						console.log(JSON.stringify(res.body));
					}
				)
			},
				...
				this.$http.post(URL,params).then(
					(res) => {
						...
						if (res.body.code == 200) {
							...
							//判断是否认证
							if (res.body.data.certifyStatus == 'Y') {
								this.showMsg({msg: '登录成功',delay: 1000});
								this.$router.push({name:'Home'});
							}else {
			...
			goCompleteInfo() {
				this.$router.push({name:'CompleteInfo',query:{active:32}});
			},
			closeRegisterModal() {
